...
/* Adds a GbWidget to the hash of known GbWidgets. The class_id is used
			     ...
			     GbWidget    *gbwidget)
{
  ...
  g_hash_table_insert (gb_widget_table, (gpointer) class_id, gbwidget);
}
		    ...
		    GbWidgetAction action, GbWidgetSetArgData * loading_data)
{
  ...
  type = gtk_type_from_name (class_id);
  ...
  if (parent) {
    ...
    if (!gb_widget_lookup (parent))
       MSG2 ("Registering unknown widget '%s' as parent of '%s'",
	     gtk_type_name (GTK_OBJECT (parent)->klass->type), class_id);
  }
			      ...
			      GbWidgetSetArgData * data)
{
  ...
  if (!gbparent)
    {
      MSG1 ("Unknown parent type %s", gtk_type_name (GTK_OBJECT (parent)->klass->type));
    }
			 ...
			 GbWidgetSetArgData * data)
{
  ...
  if (data->action == GB_LOADING)
    {
      if (applyX)
	wdata->flags |= GLADE_X_SET;
      if (applyY)
	wdata->flags |= GLADE_Y_SET;
      if (applyWidth)
  ...
  if (GTK_IS_WINDOW (widget))
	  ...
	  if (GTK_IS_FIXED (widget->parent))
	    {
		 ...
		 only use gtk_fixed_move(). */
	      ...
	      /* FIXME: GTK+ 1.2.7 bug workaround. */
	      if (data->action == GB_LOADING && GTK_WIDGET_VISIBLE (widget)) {
  ...
  else
    {
	 ...
	 make sure they aren't set. */
      wdata->flags &= ~(GLADE_X_SET | GLADE_Y_SET);
...
gb_widget_load (GtkWidget * widget, GbWidgetSetArgData * data, GtkWidget * parent)
{
...
#endif
  class_id = load_get_value (data, "class");
  MSG1 ("Reading Class: %s", class_id);
  if (!class_id)
    {
      MSG ("Load error");
      data->status = GLADE_STATUS_CLASS_ID_MISSING;
      return NULL;
    }
  ...
  if (!g_strcasecmp (class_id, "placeholder"))
    {
  ...
  while (data->token_type == GB_TOKEN_START_TAG)
      ...
      if (!strcmp (data->buffer.ptr + data->token, "widget"))
	{
	  ...
	  if (widget->parent && !gb_widget_lookup (widget->parent))
		    ...
		    gtk_type_name (GTK_OBJECT (widget->parent)->klass->type));
	    }
  ...
  if (widget_data)
    {
      ...
      save_newline (data);
      save_start_tag (data, "widget");
      save_string (data, "class", class_id);
      if (child_name)
	save_string (data, "child_name", child_name);
			 ...
			 GbWidgetGetArgData * data)
{
  ...
  name = gtk_widget_get_name (widget);
  class_id = gb_widget_get_class_id (widget);
  if (action != GB_SAVING || strcmp (name, class_id))
  ...
  /* FIXME: Tooltips do actually work for GtkCheckButton & GtkRadioButton,
     since they either have a normal window like GtkButton, or an InputOnly
     window which still works OK. */
#if 0
  if (!GTK_WIDGET_NO_WINDOW (widget))
#else
  if (!GTK_WIDGET_NO_WINDOW (widget) || GTK_IS_CHECK_BUTTON (widget))
#endif
    {
...
{ guint i;
  ...
  if (action != GB_SAVING || wdata->cxx_use_heap == 0)
			 ...
			 GbWidgetGetArgData * data)
{
  ...
  if (GTK_IS_MENU (widget))
	...
	{
	  if (wdata->flags & GLADE_X_SET)
	    gb_widget_output_int (data, GbX, wdata->x);
	  if (wdata->flags & GLADE_Y_SET)
	    gb_widget_output_int (data, GbY, wdata->y);
  ...
  if (data->action == GB_SHOWING)
    {
      if (xy_applicable)
	{
	  gboolean xsensitive = (wdata->flags & GLADE_X_SET) ? TRUE : FALSE;
	  gboolean ysensitive = (wdata->flags & GLADE_Y_SET) ? TRUE : FALSE;
...
save_style (GtkWidget * widget, GbWidgetGetArgData * data)
{
  ...
  if (wdata->gbstyle != gb_widget_default_gb_style)
    {
      ...
      if (!(wdata->flags & GLADE_STYLE_IS_UNNAMED))
	{
  ...
  if (!(wdata->flags & GLADE_STYLE_PROPAGATE))
    save_bool (data, GbStylePropagate, wdata->flags & GLADE_STYLE_PROPAGATE);
}
