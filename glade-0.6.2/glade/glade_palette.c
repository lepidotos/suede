...
glade_palette_class_init (GladePaletteClass * klass)
{
  ...
  glade_palette_signals[UNSELECT_ITEM] =
    gtk_signal_new ("unselect_item",
		    ...
		    gtk_marshal_NONE__POINTER,
		    GTK_TYPE_NONE, 1, GTK_TYPE_STRING);
  gtk_object_class_add_signals (object_class, glade_palette_signals,
				LAST_SIGNAL);
...
glade_palette_init (GladePalette * palette)
{
  ...
  palette->widget_button_group = gtk_radio_button_group (GTK_RADIO_BUTTON (palette->selector));
			  ...
			  const gchar  *tooltip)
{
  ...
  palette->widget_button_group = gtk_radio_button_group (GTK_RADIO_BUTTON (button));
	     ...
	     const gchar * section)
{
  ...
  gtk_notebook_append_page (GTK_NOTEBOOK (palette->notebook), sect->table,
			    NULL);
  sect->page = g_list_length (GTK_NOTEBOOK (palette->notebook)->children) - 1;
  button = gtk_radio_button_new_with_label (palette->section_button_group,
					    section);
  gtk_widget_set_usize (button, 20, -1);
  gtk_toggle_button_set_mode (GTK_TOGGLE_BUTTON (button), FALSE);
  palette->section_button_group = gtk_radio_button_group (GTK_RADIO_BUTTON (button));
  gtk_widget_show (button);
			   ...
			   GladePalette * palette)
{
  ...
  sect = (GladePaletteSection*) gtk_object_get_data (GTK_OBJECT (button),
						     "section");
  gtk_notebook_set_page (GTK_NOTEBOOK (palette->notebook), sect->page);
}
