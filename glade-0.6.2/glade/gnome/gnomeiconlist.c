...
gb_gnome_icon_list_new (GbWidgetNewData *data)
{
  ...
  image = gdk_imlib_create_image_from_xpm_data (gnome_iconlist_xpm);
  for (i = 0; i < 10; i++)
    {
      gnome_icon_list_append_imlib (GNOME_ICON_LIST (new_widget), image,
				 ...
				 GbWidgetWriteSourceData * data)
{
  ...
  if (data->create_widget)
	...
	flags = "0";
      ...
      source_add (data, "  %s = gnome_icon_list_new_flags (%i, NULL, %s);\n",
