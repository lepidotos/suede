...
gb_gnome_date_edit_get_properties (GtkWidget *widget, GbWidgetGetArgData * data)
{
  ...
  gb_widget_output_int (data, LowerHour, GNOME_DATE_EDIT (widget)->lower_hour);
  gb_widget_output_int (data, UpperHour, GNOME_DATE_EDIT (widget)->upper_hour);
}
...
gb_gnome_date_edit_set_properties (GtkWidget * widget, GbWidgetSetArgData * data)
{
  ...
  old_lower_hour = lower_hour = GNOME_DATE_EDIT (widget)->lower_hour;
  old_upper_hour = upper_hour = GNOME_DATE_EDIT (widget)->upper_hour;
  ...
  if (lower_hour != old_lower_hour || upper_hour != old_upper_hour)
    {
      gnome_date_edit_set_popup_range (GNOME_DATE_EDIT (widget), lower_hour,
				       upper_hour);
      /* FIXME: GnomeLibs 1.0.1 bug workaround - we change the 24 hour option
	 temporarily so that the menu is recreated. */
      if (flags & GNOME_DATE_EDIT_24_HR)
	{
	  gnome_date_edit_set_flags (GNOME_DATE_EDIT (widget),
				     flags & ~GNOME_DATE_EDIT_24_HR);
	}
      else
	{
...
gb_gnome_date_edit_write_source (GtkWidget * widget, GbWidgetWriteSourceData * data)
{
  ...
  if (GNOME_DATE_EDIT (widget)->lower_hour != 7
      || GNOME_DATE_EDIT (widget)->upper_hour != 19)
    {
      source_add (data, "  gnome_date_edit_set_popup_range (GNOME_DATE_EDIT (%s), %i, %i);\n",
		  data->wname,
		  GNOME_DATE_EDIT (widget)->lower_hour,
		  GNOME_DATE_EDIT (widget)->upper_hour);
    }
}
