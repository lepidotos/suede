...
#include "../graphics/gnome-druid-page-finish.xpm"
...
static gchar *BackgroundColor = "GnomeDruidPageFinish::background_color";
static gchar *LogoBackgroundColor = "GnomeDruidPageFinish::logo_background_color";
static gchar *TextboxColor = "GnomeDruidPageFinish::textbox_color";
static gchar *TextColor = "GnomeDruidPageFinish::text_color";
static gchar *TitleColor = "GnomeDruidPageFinish::title_color";
static gchar *Text = "GnomeDruidPageFinish::text";
static gchar *Title = "GnomeDruidPageFinish::title";
static gchar *LogoImage = "GnomeDruidPageFinish::logo_image";
static gchar *WatermarkImage = "GnomeDruidPageFinish::watermark_image";
...
/*
 * Creates a new GtkWidget of class GnomeDruidPageFinish, performing any specialized
 * initialization needed for the widget to work correctly in this environment.
 * If a dialog box is used to initialize the widget, return NULL from this
 * function, and call data->callback with your new widget when it is done.
 */
static GtkWidget*
gb_gnome_druid_page_finish_new (GbWidgetNewData *data)
{
  GtkWidget *new_widget;

  new_widget = gnome_druid_page_finish_new_with_vals ("", "", NULL, NULL);
  return new_widget;
}
 ...
 */
static void
gb_gnome_druid_page_finish_create_properties (GtkWidget * widget,
 ...
 */
static void
gb_gnome_druid_page_finish_get_properties (GtkWidget *widget,
					   GbWidgetGetArgData * data)
{
  GnomeDruidPageFinish *page;
	
  page = GNOME_DRUID_PAGE_FINISH (widget);
...
static void
gb_gnome_druid_page_finish_set_properties (GtkWidget * widget,
					   GbWidgetSetArgData * data)
{
  ...
  if (data->apply)
    gnome_druid_page_finish_set_title (page, string);

  string = gb_widget_input_text (data, Text);
  if (data->apply)
    gnome_druid_page_finish_set_text (page, string);
  if (data->action == GB_APPLYING)
    g_free (string);

  color = gb_widget_input_color (data, BackgroundColor);
  if (data->apply)
    gnome_druid_page_finish_set_bg_color (page, color);

  color = gb_widget_input_color (data, LogoBackgroundColor);
  if (data->apply)
    gnome_druid_page_finish_set_logo_bg_color (page, color);

  color = gb_widget_input_color (data, TextboxColor);
  if (data->apply)
    gnome_druid_page_finish_set_textbox_color (page, color);

  color = gb_widget_input_color (data, TextColor);
  if (data->apply)
    gnome_druid_page_finish_set_text_color (page, color);

  color = gb_widget_input_color (data, TitleColor);
  if (data->apply)
    gnome_druid_page_finish_set_title_color (page, color);
 ...
 * Add commands to aid in editing a GnomeDruidPageFinish, with signals pointing to
 * other functions in this file.
 */
/*
static void
gb_gnome_druid_page_finish_create_popup_menu (GtkWidget * widget, GbWidgetCreateMenuData * data)
{
 ...
 */
static void
gb_gnome_druid_page_finish_write_source (GtkWidget * widget,
					 GbWidgetWriteSourceData * data)
{
  ...
  if (data->create_widget)
    {
      source_add (data, "  %s = gnome_druid_page_finish_new ();\n",
		  data->wname);
    }
  ...
  if (!gdk_color_equal (color, &default_background_color))
    {
      ...
      source_add (data,
		  "  gnome_druid_page_finish_set_bg_color (GNOME_DRUID_PAGE_FINISH (%s), &%s_bg_color);\n",
  ...
  if (!gdk_color_equal (color, &default_textbox_color))
    {
      source_add_decl (data,
		       ...
		       color->red, color->green, color->blue);
      source_add (data,
		  "  gnome_druid_page_finish_set_textbox_color (GNOME_DRUID_PAGE_FINISH (%s), &%s_textbox_color);\n",
		  data->wname, data->real_wname);
    }
  ...
  if (!gdk_color_equal (color, &default_logo_background_color))
    {
      source_add_decl (data,
		       ...
		       color->red, color->green, color->blue);
      source_add (data,
		  "  gnome_druid_page_finish_set_logo_bg_color (GNOME_DRUID_PAGE_FINISH (%s), &%s_logo_bg_color);\n",
		  data->wname, data->real_wname);
    }
  ...
  if (!gdk_color_equal (color, &default_title_color))
    {
      source_add_decl (data,
		       ...
		       color->red, color->green, color->blue);
      source_add (data,
		  "  gnome_druid_page_finish_set_title_color (GNOME_DRUID_PAGE_FINISH (%s), &%s_title_color);\n",
		  data->wname, data->real_wname);
    }
  ...
  if (!gdk_color_equal (color, &default_text_color))
    {
      source_add_decl (data,
		       ...
		       color->red, color->green, color->blue);
      source_add (data,
		  "  gnome_druid_page_finish_set_text_color (GNOME_DRUID_PAGE_FINISH (%s), &%s_text_color);\n",
		  data->wname, data->real_wname);
    }
  ...
  if (strcmp (page->title, ""))
    {
      source_add (data,
		  "  gnome_druid_page_finish_set_title (GNOME_DRUID_PAGE_FINISH (%s), %s);\n",
		  data->wname,
  ...
  if (strcmp (page->text, ""))
    {
      source_add (data,
		  "  gnome_druid_page_finish_set_text (GNOME_DRUID_PAGE_FINISH (%s), %s);\n",
		  data->wname,
  ...
  if (filename && filename[0])
    {
      source_add (data,
		  "  gnome_druid_page_finish_set_logo (GNOME_DRUID_PAGE_FINISH (%s),\n"
		  "                                   create_image (\"%s/%s\"));\n",
  ...
  if (filename && filename[0])
    {
      source_add (data,
		  "  gnome_druid_page_finish_set_watermark (GNOME_DRUID_PAGE_FINISH (%s),\n"
		  "                                        create_image (\"%s/%s\"));\n",
		  data->wname, data->program_name, g_basename (filename));
    }
}
 ...
 */
GbWidget*
gb_gnome_druid_page_finish_init ()
{
  ...
  gbwidget.pixmap_struct = gnome_druid_page_finish_xpm;
  gbwidget.tooltip = _("Druid Finish Page");

  /* Fill in any functions that this GbWidget has */
  gbwidget.gb_widget_new		= gb_gnome_druid_page_finish_new;
  gbwidget.gb_widget_create_properties	= gb_gnome_druid_page_finish_create_properties;
  gbwidget.gb_widget_get_properties	= gb_gnome_druid_page_finish_get_properties;
  gbwidget.gb_widget_set_properties	= gb_gnome_druid_page_finish_set_properties;
  gbwidget.gb_widget_write_source	= gb_gnome_druid_page_finish_write_source;
/*
  gbwidget.gb_widget_create_popup_menu	= gb_gnome_druid_page_finish_create_popup_menu;
*/
