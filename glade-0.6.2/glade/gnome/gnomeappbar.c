...
gb_gnome_appbar_get_properties (GtkWidget *widget, GbWidgetGetArgData * data)
{
  gb_widget_output_bool (data, HasProgress,
			 GTK_WIDGET_VISIBLE (GNOME_APPBAR (widget)->progress));
  gb_widget_output_bool (data, HasStatus,
			 GTK_WIDGET_VISIBLE (GNOME_APPBAR (widget)->status));
}
...
gb_gnome_appbar_set_properties (GtkWidget * widget, GbWidgetSetArgData * data)
{
  ...
  if (data->apply)
    {
      if (has_progress)
	gtk_widget_show (GNOME_APPBAR (widget)->progress);
      else
	gtk_widget_hide (GNOME_APPBAR (widget)->progress);
    }
  ...
  if (data->apply)
    {
      if (has_status)
	gtk_widget_show (GNOME_APPBAR (widget)->status);
      else
	gtk_widget_hide (GNOME_APPBAR (widget)->status);
