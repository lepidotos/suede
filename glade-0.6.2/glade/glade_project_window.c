...
{
  GtkWidget *dialog;
  const gchar *buttons[] = { N_("Quit"), N_("Cancel") };
  GtkSignalFunc handlers[] = { glade_project_window_on_quit_ok, NULL };

  dialog = glade_util_create_dialog_with_buttons (_("Are you sure you want to quit?"), 2, buttons, 2, handlers, project_window, project_window->window);
  gtk_window_set_position (GTK_WINDOW (dialog), GTK_WIN_POS_MOUSE);
  gtk_window_set_transient_for (GTK_WINDOW (dialog),
				GTK_WINDOW (project_window->window));
  gtk_window_set_modal (GTK_WINDOW (dialog), TRUE);
			    ...
			    gpointer   data)
{
  ...
  href = gnome_href_new ("http://glade.pn.org/",
			 _("Glade Home Page"));
