...
struct _GbLoadPropControl
{
  GbLoadedProperty *ptr;  /* pointer to property */
  gint		 n;       /* number of buffers   */
  gint		 space;   /* max buffer space    */
};
typedef struct _GbLoadPropControl GbLoadPropControl;

#define GB_LOADED_PROPERTIES_INC 16

/* FIXME: Use GString everywhere instead. */
struct _GbBuffControl
{
  gchar      	*ptr;     /* pointer to buffer   */
  gint		 pos;     /* position in buffer  */
  gint		 space;   /* max buffer space    */
};
typedef struct _GbBuffControl GbBuffControl;
...
struct _GbWidgetSetArgData {
  ...
  /* These are only used when loading. If we are loading from a file, filename
   and fp are used. If we are pasting from the clipboard, xml_buffer_*,
  ...
  gint		     line_number;
  GbBuffControl	     buffer;
  GbTokenType	     token_type;
  gint		     token;
  GladeStatusCode    status;
  GbLoadPropControl  properties;
  GbLoadPropControl  child_properties;  
...
void        gb_init_buffer_struct (int pos, int space, GbBuffControl * p);
