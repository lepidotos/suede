...
static gchar *Policy = "HScale|GtkRange::policy";
...
gb_hscale_get_properties (GtkWidget * widget, GbWidgetGetArgData * data)
{
  ...
  gb_widget_output_int (data, Digits, GTK_RANGE (widget)->digits);

  for (i = 0; i < sizeof (GbPolicyValues) / sizeof (GbPolicyValues[0]); i++)
    {
      if (GbPolicyValues[i] == GTK_RANGE (widget)->policy)
...
gb_hscale_write_source (GtkWidget * widget, GbWidgetWriteSourceData * data)
{
  ...
  if (GTK_RANGE (widget)->digits != 1)
    source_add (data, "  gtk_scale_set_digits (GTK_SCALE (%s), %i);\n",
		data->wname, GTK_RANGE (widget)->digits);

  if (GTK_RANGE (widget)->policy != GTK_UPDATE_CONTINUOUS)
    {
      for (i = 0; i < sizeof (GbPolicyValues) / sizeof (GbPolicyValues[0]); i
	   ++)
	{
