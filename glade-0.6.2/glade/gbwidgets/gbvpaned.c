...
static gchar *Handle = "VPaned|GtkPaned::handle_size";
static gchar *Gutter = "VPaned|GtkPaned::gutter_size";
...
{
  property_add_int_range (Handle, _("Handle Size:"), _("The size of the handle"),
			  1, 1000, 1, 10, 1);
  property_add_int_range (Gutter, _("Gutter Size:"), _("The size of the gutter"),
			  1, 1000, 1, 10, 1);
...
gb_vpaned_get_properties (GtkWidget * widget, GbWidgetGetArgData * data)
{
  ...
  gb_widget_output_int (data, Handle, GTK_PANED (widget)->handle_size);
  gb_widget_output_int (data, Gutter, GTK_PANED (widget)->gutter_size);
  position = GPOINTER_TO_INT (gtk_object_get_data (GTK_OBJECT (widget),
						   Position));
...
{
  gint handle, gutter, position;

  handle = gb_widget_input_int (data, Handle);
  if (data->apply)
    gtk_paned_set_handle_size (GTK_PANED (widget), handle);

  gutter = gb_widget_input_int (data, Gutter);
  if (data->apply)
    gtk_paned_set_gutter_size (GTK_PANED (widget), gutter);
...
gb_vpaned_write_source (GtkWidget * widget, GbWidgetWriteSourceData * data)
{
  ...
  handle = GTK_PANED (widget)->handle_size;
  if (handle != 10)
    {
      source_add (data, "  gtk_paned_set_handle_size (GTK_PANED (%s), %d);\n",
		  data->wname, handle);
    }

  gutter = GTK_PANED (widget)->gutter_size;
  if (gutter != 6)
    {
      source_add (data, "  gtk_paned_set_gutter_size (GTK_PANED (%s), %d);\n",
		  data->wname, gutter);
    }
