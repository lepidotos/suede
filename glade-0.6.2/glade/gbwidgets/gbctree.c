...
static gchar *CTreeTitle = "CTree:title";
...
/* This is only used for loading & saving - it isn't displayed in the property
   editor. */
static gchar *Cols = "GtkCTree::columns";
...
new_label (GtkWidget * parent)
{
  ...
  gb_widget_set_child_name (label, CTreeTitle);
  return label;
}
		    ...
		    GbWidgetSetArgData *data)
{
  ...
  if (child_name && (!strcmp (child_name, CTreeTitle)))
    {
    ...
    {
      g_warning ("Adding unknown child to a GtkCTree");
