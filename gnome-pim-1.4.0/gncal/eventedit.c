...
static gint ee_validity_check (EventEditor *ee);
...
{
	if (!ee_validity_check (ee)) {
		GtkWidget *mbox;
		mbox = gnome_message_box_new (_("An event's end time must be later than its start time."),
					      GNOME_MESSAGE_BOX_ERROR, GNOME_STOCK_BUTTON_OK, NULL);
		gnome_dialog_set_parent (GNOME_DIALOG (mbox), GTK_WINDOW (ee));
		gnome_dialog_run (GNOME_DIALOG (mbox));	
		return;
	}
	...
	ee->ical->new = 0;
	gnome_dialog_close (GNOME_DIALOG (ee));
}
...
static gint
ee_validity_check (EventEditor *ee)
{
	time_t start, end;
...
ee_rp_init_rule (EventEditor *ee)
{
	...
	gtk_notebook_append_page (notebook, none, gtk_label_new ("None"));
	gtk_notebook_append_page (notebook, daily, gtk_label_new ("Daily"));
	gtk_notebook_append_page (notebook, weekly, gtk_label_new ("Weekly"));
	gtk_notebook_append_page (notebook, monthly, gtk_label_new ("Monthly"));
	gtk_notebook_append_page (notebook, yearly, gtk_label_new ("Yearly"));
...
ee_rp_init_ending_date (EventEditor *ee)
{
	...
	if (ee->ical->recur) {
		if (ee->ical->recur->_enddate == 0) {
			...
			gnome_date_edit_set_time (GNOME_DATE_EDIT (ee->recur_ed_end_on), ee->ical->recur->enddate-86400);
