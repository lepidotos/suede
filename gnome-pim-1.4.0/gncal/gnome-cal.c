...
gnome_calendar_direction (GnomeCalendar *gcal, int direction)
{
	...
	if (cp == gcal->day_view)
		new_time = time_add_day (time_day_begin (gcal->current_day_display), 1 * direction);
	else if (cp == gcal->week_view)
		new_time = time_add_week (time_week_begin (gcal->current_week_display), 1 * direction);
	else if (cp == gcal->month_view)
		new_time = time_add_month (time_month_begin (gcal->current_month_display), 1 * direction);
	else if (cp == gcal->year_view_sw)
		new_time = time_add_year (time_year_begin (gcal->current_year_display), 1 * direction);
	else {
...
gnome_calendar_new (char *title)
{
	...
	gcal->current_day_display = gcal->current_week_display = gcal->current_month_display = 
		gcal->current_year_display =time_day_begin (time (NULL));
...
static void
alarm_dialog_clicked (GnomeDialog* dialog, int button, gpointer data)
{
	time_t now, diff;
	CalendarAlarm *which = gtk_object_get_data (GTK_OBJECT (dialog),
						    "cal_alarm");
	iCalObject *ico = (iCalObject*) data;
	
	gboolean is_audio = (&ico->aalarm == which);

	switch (button) {
	case 1:
		if (is_audio)
			stop_beeping (GTK_OBJECT (dialog), NULL);
		now = time (NULL);
...
static gboolean
alarm_dialog_closed (GnomeDialog* dialog, gpointer data)
{
	CalendarAlarm *which = gtk_object_get_data (GTK_OBJECT (dialog),
						    "cal_alarm");
	iCalObject *ico = (iCalObject*) data;
...
calendar_notify (time_t activation_time, CalendarAlarm *which, void *data)
{
	...
	if (&ico->aalarm == which){
		...
		gtk_object_set_data (GTK_OBJECT (w), "cal_alarm", which);
		gtk_signal_connect (GTK_OBJECT (w), "clicked",
				    GTK_SIGNAL_FUNC (alarm_dialog_clicked),
				    data);
		gtk_signal_connect (GTK_OBJECT (w), "close",
				    GTK_SIGNAL_FUNC (alarm_dialog_closed),
				    data);
		gtk_widget_show (w);
		return;
	}
...
gnome_calendar_current_display (GnomeCalendar *gcal)
{
	...
	page = get_current_page (gcal);
	if (page == gcal->day_view)
		return gcal->current_day_display;
	if (page == gcal->week_view)
		return gcal->current_week_display;
	if (page == gcal->month_view)
		return gcal->current_month_display;
	if (page == gcal->year_view_sw)
		return gcal->current_year_display;
	g_assert_not_reached();
	return -1;
}
