...
static gint update_calendar_object (Child* child);
...
static gint
update_calendar_object (Child* child)
{
	GncalFullDay *fullday;
	char *text;
	...
	text = gtk_editable_get_chars (GTK_EDITABLE (child->widget), 0, -1);
...
child_focus_out (GtkWidget *widget, GdkEventFocus *event, gpointer data)
{
	...
	return update_calendar_object (child);
}
