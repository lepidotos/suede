...
/* $Id: calendar-conduit.c,v 1.29 2000/12/03 03:26:46 eskil Exp $ */
		       ...
		       GCalConduitContext *ctxt) 
{
	...
	/* This is eerie, sometimes the gnomecal doesn't start, so e
           try three more times, sepped by 1 sec */
	...
	if (ctxt->calendar == CORBA_OBJECT_NIL) {
		/* lots of people have problems with gnomecal not starting up... 
		   so we try again... */
		g_message("Could not start gnomecal server");
		return -1;
	}
			...
			iCalObject *in_obj)
{
	...
	{
		char *tmp;
		/* turn No-Time events into events at 8 o'clock with a ALLDAY: marking */
		a.begin.tm_sec = 0;
		a.begin.tm_min = 0;
		a.begin.tm_hour = 8;
		...
		a.end.tm_min = 0;
		a.end.tm_hour = 9;
		tmp = g_strdup_printf ("ALLDAY: %s", obj->summary);
		g_free (obj->summary);
		obj->summary = tmp;
	}
	...
	*/
	 ...
	 * Recurrence
	 */
	if (a.repeatFrequency && a.repeatType != repeatNone){
		...
		case repeatYearly:
			...
			break;
		default:
			g_warning("Unknown repeat type, malformed calendar ? (type was %d)",a.repeatType);
		...
		if (a.repeatForever)
			...
			/* Add one day (86400 secs to make gnomecal see the end-on-date */
			g_message("repeatend is %d/%d/%d",a.repeatEnd.tm_mday,a.repeatEnd.tm_mon,1900+a.repeatEnd.tm_year);
			obj->recur->_enddate = mktime (&a.repeatEnd)+86400;
		}
	  ...
	  GCalConduitContext *ctxt)
{
	...
	if (local->ical->recur != NULL) {
		...
		if (local->ical->recur->duration == 0) {
			...
			time_t tmptime = local->ical->recur->_enddate - 86400; 
			local->a->repeatForever = 0;
			local->a->repeatEnd = *localtime(&tmptime);
		}
