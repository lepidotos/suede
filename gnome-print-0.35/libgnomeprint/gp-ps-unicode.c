 ...
 *
 * TODO:
 * 1) Use more meaningful data structures instead of hash tables
 * 2) Really rethink, how that all works wor multinames/multicodes
 *
 */
...
/* We maintain separate lookup tables for standard adobe glyph names and for dingbats */
static GHashTable * ps2uni = NULL;
static GHashTable * db2uni = NULL;
/* Unicode to postscript mapping, unified */
static GHashTable * uni2ps = NULL;
/* return GSList of unicode values associated with given ps name */
static GHashTable * ps2multi = NULL;
/* The last one simply maps ps name to static ps name */
static GHashTable * ps2ps = NULL;

static void gen_tables (void);

gint
gp_unicode_from_ps (const gchar * name)
{
	g_return_val_if_fail (name != NULL, 0);

	if (!ps2uni) gen_tables ();

	return GPOINTER_TO_INT (g_hash_table_lookup (ps2uni, name));
}

gint
gp_unicode_from_dingbats (const gchar * name)
{
	g_return_val_if_fail (name != NULL, 0);

	if (!db2uni) gen_tables ();

	return GPOINTER_TO_INT (g_hash_table_lookup (db2uni, name));
}

/*
 * fixme:
 *
 * 1) We do not handle surrogates
 * 2) Saving const strings to dictionary will eventually eat all our memory
 *
 */

const gchar *
gp_ps_from_unicode (gint unicode)
{
	static GHashTable * std2ps = NULL;
	gchar * ps;

	if (!uni2ps) gen_tables ();

	ps = g_hash_table_lookup (uni2ps, GINT_TO_POINTER (unicode));

	if (!ps) {
		if (!std2ps) std2ps = g_hash_table_new (NULL, NULL);
		ps = g_hash_table_lookup (std2ps, GINT_TO_POINTER (unicode));
		if (!ps) {
			ps = g_strdup_printf ("uni%.4X", unicode);
			g_hash_table_insert (std2ps, GINT_TO_POINTER (unicode), ps);
		}
	}
...
const gchar *
gp_const_ps_from_ps (const gchar * name)
{
	g_return_val_if_fail (name != NULL, NULL);

	if (!ps2ps) gen_tables ();

	return g_hash_table_lookup (ps2ps, name);
}

const GSList *
gp_multi_from_ps (const gchar * name)
{
	g_return_val_if_fail (name != NULL, NULL);

	if (!ps2multi) gen_tables ();

	return g_hash_table_lookup (ps2multi, name);
}
...
static gint gp_univalue (const gchar * n, gint len)
{
	/* Step 3 */
	if (len == 7) {
		if ((n[0] == 'u') && (n[1] == 'n') && (n[2] == 'i')) {
			gint i;
			for (i = 3; i < 7; i++) {
				if (!isxdigit (n[i])) break;
				if (n[i] < 'A') break; /* Faster than isupper and should work */
			}
			if (i == 7) {
				gint unicode;
				unicode = strtol (n + 3, NULL, 16);
				return unicode;
			}
		}
	}
...
GSList *
gp_ps_get_semantics (const gchar * name, gboolean * isDecomp, gboolean * isVar)
{
	gchar n[256];
	gchar * p;
	GSList * ml;
	...
	g_return_val_if_fail (name != NULL, NULL);
	g_return_val_if_fail (isDecomp != NULL, NULL);
	g_return_val_if_fail (isVar != NULL, NULL);
	...
	*isVar = FALSE;
	...
	/* Step 2 */
	ml = (GSList *) gp_multi_from_ps (n);
	if (ml) return g_slist_copy (ml);
	unicode = gp_unicode_from_ps (n);
	if (unicode > 0) return g_slist_prepend (NULL, GINT_TO_POINTER (unicode));

	/* Step 3 */
	len = strlen (n);
	unicode = gp_univalue (n, len);
	if (unicode) return g_slist_prepend (NULL, GINT_TO_POINTER (unicode));
	...
	*isDecomp = TRUE;
	for (p = n; p != NULL; p = strchr (p, '_')) {
		...
		if (!e) e = p + strlen (p);
		l = e - p;
		/* Step 2 with single value */
		/* fixme: Think out, whether we should somehow handle multiple codepoints */
		strncpy (c, p, l);
		c[l] = '\0';
		unicode = gp_unicode_from_ps (c);
		if (!unicode) {
			/* Step 3 */
			unicode = gp_univalue (c, l);
		}
		if (unicode) {
			ml = g_slist_prepend (ml, GINT_TO_POINTER (unicode));
		} else {
			/* Found nonexistent sequence */
			g_slist_free (ml);
			ml = NULL;
			break;
		}
	...
	/* Step 5 */
	if ((strncmp (n, "uni", 3) == 0) && ((len & 0x07) == 3)) {
		gchar c[5];
		gint i;
		/* We are candidate for uniABCDEFGH... composite */
		for (i = 3; i < len; i++) {
			if (!isxdigit (n[i])) break;
			if (n[i] < 'A') break; /* Faster than isupper and should work */
		}
		if (i == len) {
			/* We are not valid sequence */
			c[4] = '\0';
			for (i = 3; i < len; i+= 4) {
				strncpy (c, n + i, 4);
				unicode = strtol (n + 3, NULL, 16);
...
gen_tables (void)
{
	...
	ps2uni = g_hash_table_new (g_str_hash, g_str_equal);
	db2uni = g_hash_table_new (g_str_hash, g_str_equal);
	uni2ps = g_hash_table_new (NULL, NULL);
	ps2ps = g_hash_table_new (g_str_hash, g_str_equal);
	ps2multi = g_hash_table_new (g_str_hash, g_str_equal);

	g_hash_table_freeze (ps2uni);
	g_hash_table_freeze (db2uni);
	g_hash_table_freeze (uni2ps);
	g_hash_table_freeze (ps2ps);
	g_hash_table_freeze (ps2multi);

	for (i = 0; i < unitabs; i++) {
		if (!g_hash_table_lookup (ps2uni, unitab[i].name)) {
			g_hash_table_insert (ps2uni, unitab[i].name, GINT_TO_POINTER (unitab[i].unicode));
		} else {
			GSList * l;
			l = g_hash_table_lookup (ps2multi, unitab[i].name);
			if (!l) l = g_slist_prepend (l, g_hash_table_lookup (ps2uni, unitab[i].name));
			l = g_slist_prepend (l, GINT_TO_POINTER (unitab[i].unicode));
			g_hash_table_insert (ps2multi, unitab[i].name, l);
		}
		if (!g_hash_table_lookup (uni2ps, GINT_TO_POINTER (unitab[i].unicode))) {
			g_hash_table_insert (uni2ps, GINT_TO_POINTER (unitab[i].unicode), unitab[i].name);
		}
		g_hash_table_insert (ps2ps, unitab[i].name, unitab[i].name);
	}

	for (i = 0; i < dingtabs; i++) {
		g_hash_table_insert (db2uni, dingtab[i].name, GINT_TO_POINTER (dingtab[i].unicode));
		if (!g_hash_table_lookup (uni2ps, GINT_TO_POINTER (dingtab[i].unicode))) {
			g_hash_table_insert (uni2ps, GINT_TO_POINTER (dingtab[i].unicode), dingtab[i].name);
		}
		g_hash_table_insert (ps2ps, dingtab[i].name, dingtab[i].name);
	}

	g_hash_table_thaw (ps2uni);
	g_hash_table_thaw (db2uni);
	g_hash_table_thaw (uni2ps);
	g_hash_table_thaw (ps2ps);
	g_hash_table_thaw (ps2multi);
}
