/* -*- Mode: C; indent-tabs-mode: t; c-basic-offset: 8; tab-width: 8 -*-
   ...
   list-list-column-title.c: List column title widget for interacting with list columns
...
struct EelListColumnTitleDetails 
{
...
static void eel_list_column_title_initialize_class	(gpointer klass);
static void eel_list_column_title_initialize       (gpointer object, gpointer klass);
static void eel_list_column_title_paint		(GtkWidget *widget, GtkWidget *draw_target, GdkDrawable *target_drawable, GdkRectangle *area);
static void eel_list_column_title_draw 		(GtkWidget *widget, GdkRectangle *box);
static void eel_list_column_title_buffered_draw	(GtkWidget *widget);
static void eel_list_column_title_queue_buffered_draw(GtkWidget *widget);
static gboolean eel_list_column_title_expose 	(GtkWidget *widget, GdkEventExpose *event);
static void eel_list_column_title_realize 		(GtkWidget *widget);
static void eel_list_column_title_finalize 	(GtkObject *object);
static void eel_list_column_title_request 		(GtkWidget *widget, GtkRequisition *requisition);

static gboolean eel_list_column_title_motion	(GtkWidget *widget, GdkEventMotion *event);
static gboolean eel_list_column_title_leave 	(GtkWidget *widget, GdkEventCrossing *event);

static gboolean eel_list_column_title_button_press	(GtkWidget *widget, GdkEventButton *event);
static gboolean eel_list_column_title_button_release (GtkWidget *widget, GdkEventButton *event);


EEL_DEFINE_CLASS_BOILERPLATE (EelListColumnTitle, eel_list_column_title, GTK_TYPE_BIN)
/* generates eel_list_column_title_get_type */

static void
eel_list_column_title_initialize_class (gpointer klass)
{
	...
	object_class->finalize = eel_list_column_title_finalize;
	widget_class->draw = eel_list_column_title_draw;
	widget_class->expose_event = eel_list_column_title_expose;
	widget_class->realize = eel_list_column_title_realize;
	widget_class->size_request = eel_list_column_title_request;
	widget_class->motion_notify_event = eel_list_column_title_motion;
	widget_class->leave_notify_event = eel_list_column_title_leave;
	widget_class->button_press_event = eel_list_column_title_button_press;
	widget_class->button_release_event = eel_list_column_title_button_release;
}

EelListColumnTitle *
eel_list_column_title_new (void)
{
	return EEL_LIST_COLUMN_TITLE
		(gtk_widget_new (eel_list_column_title_get_type (), NULL));
}

static void
eel_list_column_title_initialize (gpointer object, gpointer klass)
{
	EelListColumnTitle *column_title;

	column_title = EEL_LIST_COLUMN_TITLE(object);
	column_title->details = g_new0 (EelListColumnTitleDetails, 1);
...
static void
eel_list_column_title_realize (GtkWidget *widget)
{
	GdkWindowAttr attributes;
	int attributes_mask;

	g_assert (EEL_IS_LIST_COLUMN_TITLE (widget));
...
static void
eel_list_column_title_finalize (GtkObject *object)
{
	EelListColumnTitle *column_title;
...
static void
eel_list_column_title_request (GtkWidget *widget, GtkRequisition *requisition)
{
...
{
	EelCList *parent_clist;

	parent_clist = EEL_CLIST (column_title->parent);
		    ...
		    GdkPixmap **indicator_pixmap, GdkBitmap **indicator_mask)
{
	...
	column_title = EEL_LIST_COLUMN_TITLE(widget);
...
/* Add more truncation modes, optimize for performance, move to eel-gdk-extensions */
static char *
...
static void
eel_list_column_title_paint (GtkWidget *widget, GtkWidget *draw_target, 
				  GdkDrawable *target_drawable, GdkRectangle *area)
{
	...
	g_assert (EEL_CLIST (widget->parent) != NULL);

	column_title = EEL_LIST_COLUMN_TITLE(widget);
	parent_clist = EEL_CLIST (widget->parent);
	...
	for (index = 0; index < parent_clist->columns; index++) {
		...
		if (!parent_clist->column[index].visible) {
			continue;
		}
...
static void
eel_list_column_title_draw (GtkWidget *widget, GdkRectangle *area)
{
	g_assert (EEL_IS_LIST_COLUMN_TITLE (widget));
	g_assert (area != NULL);
	...
	eel_list_column_title_paint (widget, widget, widget->window, area);
}

static void
eel_list_column_title_buffered_draw (GtkWidget *widget)
{
	...
	eel_list_column_title_paint (widget, widget,
...
offscreen_drawing_idle_handler (gpointer data)
{
	...
	g_assert (EEL_IS_LIST_COLUMN_TITLE (data));

	widget = GTK_WIDGET (data);
	column_title = EEL_LIST_COLUMN_TITLE (data);

	eel_list_column_title_buffered_draw (widget);
...
static void
eel_list_column_title_queue_buffered_draw (GtkWidget *widget)
{
	EelListColumnTitle *column_title;
...
static gboolean
eel_list_column_title_expose (GtkWidget *widget, GdkEventExpose *event)
{
	g_assert (EEL_IS_LIST_COLUMN_TITLE (widget));
	g_assert (event != NULL);
	...
	eel_list_column_title_paint (widget, widget, widget->window, &event->area);
...
in_column_rect (GtkWidget *widget, int x, int y)
{
	...
	for (index = 0; index < parent_clist->columns; index++) {
		 ...
		 * eel_list_column_title_leave depends on this 
		 */
...
static gboolean
eel_list_column_title_motion (GtkWidget *widget, GdkEventMotion *event)
{
	EelListColumnTitle *column_title;
	...
	g_assert (EEL_IS_LIST_COLUMN_TITLE (widget));
	g_assert (EEL_IS_LIST (widget->parent));
	...
	if (title_update_needed) {
		eel_list_column_title_queue_buffered_draw (widget);
	}
...
static gboolean
eel_list_column_title_leave (GtkWidget *widget, GdkEventCrossing *event)
{
	EelListColumnTitle *column_title;
...
static gboolean
eel_list_column_title_button_press (GtkWidget *widget, GdkEventButton *event)
{
	EelListColumnTitle *column_title;
	GtkWidget *parent_list;
	int grab_result;
	...
	g_assert (event->type != GDK_BUTTON_PRESS
		|| EEL_LIST_COLUMN_TITLE(widget)->details->tracking_column_resize == -1);
...
static gboolean 
eel_list_column_title_button_release (GtkWidget *widget, GdkEventButton *event)
{
	EelListColumnTitle *column_title;
	GtkWidget *parent_list;
...
void
eel_list_column_title_queue_draw (EelListColumnTitle *title)
{
	eel_list_column_title_queue_buffered_draw (GTK_WIDGET (title));
}
