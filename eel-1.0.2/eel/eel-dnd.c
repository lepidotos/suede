...
/* eel-drag.c - Common Drag & drop handling code shared by the icon container
   and the list view.
...
#define EEL_COMMAND_SPECIFIER "command:"
...
void
eel_drag_init (EelDragInfo *drag_info,
...
void
eel_drag_finalize (EelDragInfo *drag_info)
{
...
/* Functions to deal with EelDragSelectionItems.  */

EelDragSelectionItem *
eel_drag_selection_item_new (void)
{
	return g_new0 (EelDragSelectionItem, 1);
}

static void
drag_selection_item_destroy (EelDragSelectionItem *item)
{
	g_free (item->uri);
	g_free (item);
}
...
void
eel_drag_destroy_selection_list (GList *list)
{
	GList *p;
...
GList *
eel_drag_build_selection_list (GtkSelectionData *data)
{
	GList *result;
	const guchar *p, *oldp;
	int size;
	...
	while (size > 0) {
		...
		item = eel_drag_selection_item_new ();
...
gboolean
eel_drag_items_local (const char *target_uri_string, const GList *selection_list)
{
	...
	item_uri = gnome_vfs_uri_new (((EelDragSelectionItem *)selection_list->data)->uri);
...
gboolean
eel_drag_items_in_trash (const GList *selection_list)
{
	 ...
	 * and currently dropped items can only be from the same container
	 */
	return eel_uri_is_in_trash (((EelDragSelectionItem *)selection_list->data)->uri);
}

void
eel_drag_default_drop_action_for_icons (GdkDragContext *context,
	...
	int *default_action, int *non_default_action)
{
	...
	} else if (eel_str_has_prefix (target_uri_string, EEL_COMMAND_SPECIFIER)) {
		*default_action = GDK_ACTION_MOVE;
		*non_default_action = GDK_ACTION_MOVE;
		return;
	} else {
	...
	dropped_uri = gnome_vfs_uri_new (((EelDragSelectionItem *)items->data)->uri);
	same_fs = TRUE;
...
is_path_that_gnome_uri_list_extract_filenames_can_parse (const char *path)
{
	 ...
	 */
	if (isspace ((guchar) path [0])) {
		return FALSE;
	}
	if (isspace ((guchar) path [strlen (path) - 1])) {
		return FALSE;
	}
...
gboolean
eel_drag_drag_data_get (GtkWidget *widget,
...
int
eel_drag_modifier_based_action (int default_action, int non_default_action)
{
...
eel_drag_drop_action_ask (GdkDragAction actions)
{
	 ...
	 * allowed actions.
	 */
	saved_domain = textdomain (GNOME_EXPLICIT_TRANSLATION_DOMAIN);
	menu = gnome_popup_menu_new (menu_items);
	textdomain (saved_domain);
...
gboolean
eel_drag_autoscroll_in_scroll_region (GtkWidget *widget)
{
...
void
eel_drag_autoscroll_calculate_delta (GtkWidget *widget, float *x_scroll_delta, float *y_scroll_delta)
{
	int x, y;
...
void
eel_drag_autoscroll_start (EelDragInfo *drag_info,
				GtkWidget        *widget,
				GtkFunction       callback,
				gpointer          user_data)
{
	if (eel_drag_autoscroll_in_scroll_region (widget)) {
...
void
eel_drag_autoscroll_stop (EelDragInfo *drag_info)
{
	if (drag_info->auto_scroll_timeout_id != 0) {
		gtk_timeout_remove (drag_info->auto_scroll_timeout_id);
		drag_info->auto_scroll_timeout_id = 0;
